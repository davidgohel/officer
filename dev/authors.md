# Authors and Citation

## Authors

- **[David Gohel](https://www.ardata.fr)**. Author, maintainer.

- **Stefan Moog**. Author.

- **Mark Heckmann**. Author. [](https://orcid.org/0000-0002-0736-7417)

- **ArData**. Copyright holder.

- **Frank Hangler**. Contributor.  
  function body_replace_all_text

- **Liz Sander**. Contributor.  
  several documentation fixes

- **Anton Victorson**. Contributor.  
  fixes xml structures

- **Jon Calder**. Contributor.  
  update vignettes

- **John Harrold**. Contributor.  
  function annotate_base

- **John Muschelli**. Contributor.  
  google doc compatibility

- **Bill Denney**. Contributor.
  [](https://orcid.org/0000-0002-5759-428X)  
  function as.matrix.rpptx

- **Nikolai Beck**. Contributor.  
  set speaker notes for .pptx documents

- **Greg Leleu**. Contributor.  
  fields functionality in ppt

- **Majid Eismann**. Contributor.

- **Wahiduzzaman Khan**. Contributor.  
  vectorization of remove_slide

- **Hongyuan Jia**. Contributor.
  [](https://orcid.org/0000-0002-0075-8183)

- **Michael Stackhouse**. Contributor.

## Citation

Source:
[`DESCRIPTION`](https://github.com/davidgohel/officer/blob/main/DESCRIPTION)

Gohel D, Moog S, Heckmann M (2026). *officer: Manipulation of Microsoft
Word and PowerPoint Documents*. R package version 0.7.5.005,
<https://ardata-fr.github.io/officeverse/>.

    @Manual{,
      title = {officer: Manipulation of Microsoft Word and PowerPoint Documents},
      author = {David Gohel and Stefan Moog and Mark Heckmann},
      year = {2026},
      note = {R package version 0.7.5.005},
      url = {https://ardata-fr.github.io/officeverse/},
    }
