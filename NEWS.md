...
- `add_slide()`: It now suffices to specify a unique `layout`. The `master` will be inferred automatically.  
  Also, the defaults for `layout` and `master` were removed for more generality. They only applied to the default 
  pptx generated by `read_pptx()`. To avoid breaking changes in old code, `read_pptx()` automatically sets the default 
  layout `"Slide and Content"`, if it exists. This makes sure, that the behavior of `add_slide()` does not change (#635)
