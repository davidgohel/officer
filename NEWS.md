...
- `ph_with()` now handles `Date` objects without prior conversion to character. The date format can 
  be set locally via the `format` arg or globally via `options(officer.date_format = ...)` (#683)
