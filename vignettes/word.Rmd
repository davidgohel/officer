...
and styles are defined in the initial file.
...
* One of the styles is *heading 1* which correspond to a level 1 title. 
...
* Table templates are also defined in the initial document and can be used with `body_add_table`. For advanced tabular formatting, use package `flextable` instead ([flextable website](https://davidgohel.github.io/officer/)), it has a function `body_add_flextable()` that can be used with `officer`.
...
You can also use the `body_replace_*` functions to search-and-replace text. `body_replace_at` replaces text at a bookmark. `body_replace_all_text` will, depending on the options provided, replace text either at the cursor or in the entire document.
