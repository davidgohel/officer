...
available slide layouts will be those available in the template file. Content of original document is also preserved (but can be manipulated, i.e. delete a slide).
...
Layout names and master layout names can be read with function `layout_summary`.
...
is defined in the slide layout associated to the current slide, for example, using `type = "title"` will create a title shape in the slide.
...
> Function `layout_properties` provides details about available shapes of a slide 
layout. 
...
Argument `type` specify which placeholder of the associated 
layout is to be added (`index` is to be used when a `type` 
...
To add an image into a new shape at arbitrary coordinates, use function `ph_with_img_at`. 
...
Function `ph_add_text` has an optional argument `href`, if used, the chunk of text will be an hyperlink. If `href` is not used and `slide_index` is provided, the link will point to another slide in the document.
