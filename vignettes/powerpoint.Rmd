...
my_pres <- my_pres %>% 
  ...
  add_slide(layout = "Title and Content", master = "Office Theme") %>% 
  add_slide(layout = "Title Only", master = "Office Theme") %>% 
  move_slide(index = 1, to = 3)
```


## Add outputs in a placeholder of a slide
...
doc <- ph_with(x = doc, value = head(iris), 
   location = ph_location_type(type = "body", position_right = FALSE) )
doc <- ph_with(x = doc, value = head(mtcars), 
   location = ph_location_type(type = "body", position_right = TRUE) )
print(doc, target = "assets/pptx/ph_with_df.pptx")
```

```{r echo=FALSE}
office_doc_link( url = paste0( "https://davidgohel.github.io/officer/articles/", "assets/pptx/ph_with_df.pptx" ) )
```
...
```{r echo=FALSE}
office_doc_link( url = paste0( "https://davidgohel.github.io/officer/articles/", "assets/pptx/ph_with_gg.pptx" ) )
```
...
* Text and also vectors (`character`, `numeric`, `factor`, `logical`)
...
doc <- ph_with(x = doc, value = c("one blah", "two blah"), location = ph_location_left() )
doc <- ph_with(x = doc, value = c("three blah", "four blah"), location = ph_location_right() )
...
print(doc, target = "assets/pptx/ph_with_block_list.pptx")
```

```{r echo=FALSE}
office_doc_link( url = paste0( "https://davidgohel.github.io/officer/articles/", "assets/pptx/ph_with_block_list.pptx" ) )
```
...
print(doc, target = "assets/pptx/ph_with_fpar.pptx")
```

```{r echo=FALSE}
office_doc_link( url = paste0( "https://davidgohel.github.io/officer/articles/", "assets/pptx/ph_with_fpar.pptx" ) )
```
...
print(doc, target = "assets/pptx/ph_with_ul.pptx")
```

```{r echo=FALSE}
office_doc_link( url = paste0( "https://davidgohel.github.io/officer/articles/", "assets/pptx/ph_with_ul.pptx" ) )
```
...
> If your goal is to add formatted text in a new shape, use `ph_with(..., block_list(...))` 
instead of the following. **The following *appenders* are to be used to add something 
to an existing content, i.e. something with existing text that needs to be preserved. **
...
The following code produces a presentation with a slide made of one text shape 
