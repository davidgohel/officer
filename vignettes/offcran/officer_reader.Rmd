...
library(flextable)
library(knitr)
genere_flextable <- function(x, ...) {
  ft <- flextable(x, defaults = list(fontname = "Roboto"))
  ft <- theme_zebra(ft, odd_header = "#FCFCFC", odd_body = "#FCFCFC")
  ft <- hline_bottom(ft, border = fp_border(), part = "header")
  ft <- fontsize(ft, size = 15, part = "all")
  ft <- padding(ft, padding = 5, part = "all")
  ft <- set_formatter_type(ft, na_str = "<na>")
  ft <- set_table_properties(ft, layout = "autofit")
  knitr::knit_print(ft)
}
registerS3method("knit_print", "data.frame", genere_flextable)
...
Cells positions and values are dispatched in columns `row_num`, `cell_id`, `text`
...
table_body <- table_body[,c("row_num", "cell_id", "text")]
head(table_body)
```

Reshaping the data with columns `row_num`, `cell_id` and `text` would display 
something close to the orginal table: 

```{r}
tapply(table_body$text, 
       list(row_num = table_body$row_num, 
...
data <- data[, c("row_num", "cell_id", "text") ] 

tapply(data$text, 
   list(row_num = data$row_num, 
...
```{r}
data <- subset(table_cells, id == 18, c(row_num, cell_id, text) )
tapply(data$text, 
   list(row_num = data$row_num, 
        cell_id = data$cell_id
        ), FUN = I )
