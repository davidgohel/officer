...
A section is a grouping of blocks (ie. paragraphs and tables) that have
a set of properties that define pages on which the text will appear.
...
**Important**: When creating multiple sections in a document, it is
strongly recommended to use the **same `page_margins` object** for all
sections to avoid unwanted page breaks. Changing page margins between
sections can cause Word to insert automatic page breaks, even when using
`type = "continuous"`. To ensure consistent behavior, create a single
[`page_mar()`](https://davidgohel.github.io/officer/reference/page_mar.md)
object and reuse it across all `prop_section()` calls. See the examples
in
[`body_end_block_section()`](https://davidgohel.github.io/officer/reference/body_end_block_section.md)
which demonstrate this best practice.
...
- page_size:
  ...
  page dimensions, an object generated with function
  [page_size](https://davidgohel.github.io/officer/reference/page_size.md).

- page_margins:

  page margins, an object generated with function
  [page_mar](https://davidgohel.github.io/officer/reference/page_mar.md).
  It is recommended to use the same margins object across all sections
  to prevent unintended page breaks.

- type:

  Section type. It defines how the contents of the section will be
  placed relative to the previous section. Available types are
  "continuous" (begins the section on the next paragraph), "evenPage"
  (begins on the next even-numbered page), "nextColumn" (begins on the
  next column on the page), "nextPage" (begins on the following page),
  "oddPage" (begins on the next odd-numbered page).

- section_columns:

  section columns, an object generated with function
...
- header_default:
  ...
  for the default page header. Use NULL (default value) for no content.

- header_even:

  content as a
  [`block_list()`](https://davidgohel.github.io/officer/reference/block_list.md)
  for the even page header. Use NULL (default value) for no content.

- header_first:

  content as a
  [`block_list()`](https://davidgohel.github.io/officer/reference/block_list.md)
  for the first page header. Use NULL (default value) for no content.

- footer_default:

  content as a
  [`block_list()`](https://davidgohel.github.io/officer/reference/block_list.md)
  for the default page footer. Use NULL (default value) for no content.

- footer_even:

  content as a
  [`block_list()`](https://davidgohel.github.io/officer/reference/block_list.md)
  for the even page footer. Use NULL (default value) for no content.
...
![](figures/prop_section_doc_1.png)
