...
write_relationship <- function( x, relationships ) {

  rels_ <- sprintf("<Relationship Id=\"%s\" Type=\"%s\" Target=\"%s\"/>",
          relationships$id, relationships$type, relationships$target)
  rels_ <- paste(rels_, collapse = "")
  rel_path <- file.path(x$package_dir, "word/_rels/document.xml.rels")
  rels_ <- paste0(XML_HEADER, "\n",
         "<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">",
         rels_,
         "</Relationships>",
         "\n"
         )
  cat(rels_, file = rel_path)
}
