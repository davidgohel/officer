...
test_that("body_add_xml_multi works as expected", {
  # Empty doc
  pgs <- list(
    to_wml(fpar("Page 1"), add_ns = TRUE),
    to_wml(fpar("Page 2"), add_ns = TRUE),
    to_wml(fpar("Page 3"), add_ns = TRUE)
  )

  doc <- read_docx()
  doc <- body_add_xml_multi(doc, pgs)
  ...
  # Before
  doc <- read_docx()
  doc <- body_add_xml(doc, to_wml(fpar("Page X"), add_ns = TRUE))
  doc <- body_add_xml_multi(doc, pgs, pos = "before")
  ...
  doc <- read_docx()
  doc <- body_add_xml(doc, to_wml(fpar("Page X"), add_ns = TRUE))
  doc <- body_add_xml_multi(doc, pgs, pos = "after")
  ...
  doc <- read_docx()
  doc <- body_add_xml(doc, to_wml(fpar("Page X"), add_ns = TRUE))
  doc <- body_add_xml_multi(doc, pgs, pos = "on")
